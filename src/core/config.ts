 ...
 * 可建造区为三个等面积（6×15 格 = 90）矩形：各自包住己方出生点、互不重叠；
 * 面向质心的边界三方一致（争夺面公平），背向质心延伸到地图边缘留足纵深。
 * 旧版只给 6×8，被城堡占位、中央金矿禁建圈和岩石掏得只剩十几个合法位，
 * 玩家体感"哪都放不了"（1v1 建造区是 512 格，对比悬殊）。
 */
const SPAWNS_3P: SpawnDef[] = [
  { pos: { x: 360, y: 1203 }, build: { x0: 6, x1: 11, y0: 24, y1: 38 } },
  { pos: { x: 80, y: 718 }, build: { x0: 1, x1: 6, y0: 7, y1: 21 } },
  { pos: { x: 640, y: 718 }, build: { x0: 11, x1: 16, y0: 7, y1: 21 } },
];
